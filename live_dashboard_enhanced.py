"""
Dashboard Inteligente Mejorado para Bot MT5
...
class EnhancedLiveDashboard:
    ...
    def init_enhanced_db(self):
        ...
        # Tabla de snapshots de confianza por hora
        c.execute('''
            CREATE TABLE IF NOT EXISTS confidence_snapshots (
                ...
                medium_high_count INTEGER DEFAULT 0,
    ...
    def generate_confidence_distribution_chart(self) -> str:
        """Generar gráfico de distribución de confianza"""
        distribution = self.get_confidence_distribution()
        
        if sum(distribution.values()) == 0:
            return "<div style='text-align: center; color: #888; padding: 50px;'>No hay datos de confianza disponibles</div>"
        
        # Crear gráfico de pie
        fig = go.Figure(data=[go.Pie(
            labels=list(distribution.keys()),
            values=list(distribution.values()),
            marker_colors=[self.confidence_colors[level] for level in distribution.keys()],
        ...
        fig.update_layout(
            title="Distribución de Señales por Confianza",
        ...
        return fig.to_html(include_plotlyjs='cdn', div_id="confidence-distribution")
    
    def generate_symbol_performance_chart(self) -> str:
        """Generar gráfico de rendimiento por símbolo"""
        stats_by_symbol = self.get_symbol_statistics('ALL')
        
        if not stats_by_symbol:
            return "<div style='text-align: center; color: #888; padding: 50px;'>No hay datos de rendimiento por símbolo</div>"
        
        symbols = list(stats_by_symbol.keys())
        pnl_values = [stats_by_symbol[symbol]['total_pnl'] for symbol in symbols]
        win_rates = [stats_by_symbol[symbol]['win_rate'] for symbol in symbols]
        
        # Crear subplots
        fig = make_subplots(
            rows=1, cols=2,
            subplot_titles=('P&L por Símbolo', 'Win Rate por Símbolo'),
            specs=[[{"secondary_y": False}, {"secondary_y": False}]]
        )
        
        # Gráfico de barras P&L
        colors = ['#00ff88' if pnl >= 0 else '#ff4444' for pnl in pnl_values]
        fig.add_trace(
        ...
        fig.add_trace(
            go.Bar(x=symbols, y=win_rates, marker_color='#00ccff', name="Win Rate %"),
            row=1, col=2
        )
        ...
        return fig.to_html(include_plotlyjs='cdn', div_id="symbol-performance")
    
    def generate_signals_timeline_chart(self) -> str:
        """Generar gráfico de línea temporal de señales"""
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        
        # Obtener señales agrupadas por hora
        c.execute('''
            SELECT 
            ...
            FROM enhanced_signals 
            WHERE session_id = ?
            GROUP BY hour, confidence_level
            ORDER BY hour
        ''', (self.session_id,))
        ...
        if not data:
            return "<div style='text-align: center; color: #888; padding: 50px;'>No hay datos de timeline disponibles</div>"
        
        # Organizar datos por nivel de confianza
        df = pd.DataFrame(data, columns=['hour', 'confidence_level', 'count'])
        ...
        for level in self.confidence_levels:
            ...
            if not level_data.empty:
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines+markers',
                    name=level,
                    line=dict(color=self.confidence_colors[level], width=2),
                    marker=dict(size=6)
                ))
        
        fig.update_layout(
            title="Timeline de Señales por Confianza",
            xaxis_title="Tiempo",
        ...
        return fig.to_html(include_plotlyjs='cdn', div_id="signals-timeline")
    ...
    def generate_enhanced_dashboard_html(self, symbol_filter: str = 'ALL', 
        ...
        # Generar gráficos
        confidence_chart = self.generate_confidence_distribution_chart()
        performance_chart = self.generate_symbol_performance_chart()
        timeline_chart = self.generate_signals_timeline_chart()
        ...
        <head>
            ...
            <script>
                function applyFilters() {{
                    ...
                    // En una implementación real, esto recargaría con los filtros
                    console.log('Filtros aplicados:', {{ symbol, confidence, status }});
                    // Aquí iría la lógica para recargar con filtros
                }}
        ...
        <body>
            ...
            <div class="filters-section">
                <div class="filter-group">
                    <label>📊 Símbolo:</label>
                    <select id="symbol-filter" class="filter-select" onchange="applyFilters()">
                        <option value="ALL" {'selected' if symbol_filter == 'ALL' else ''}>Todos</option>
                        <option value="EURUSD" {'selected' if symbol_filter == 'EURUSD' else ''}>EURUSD</option>
                        <option value="XAUUSD" {'selected' if symbol_filter == 'XAUUSD' else ''}>XAUUSD</option>
                        <option value="BTCEUR" {'selected' if symbol_filter == 'BTCEUR' else ''}>BTCEUR</option>
                    </select>
                </div>
                
                <div class="filter-group">
                    <label>🧠 Confianza:</label>
                    <select id="confidence-filter" class="filter-select" onchange="applyFilters()">
                        <option value="ALL" {'selected' if confidence_filter == 'ALL' else ''}>Todas</option>
                        <option value="HIGH" {'selected' if confidence_filter == 'HIGH' else ''}>HIGH</option>
                        <option value="MEDIUM-HIGH" {'selected' if confidence_filter == 'MEDIUM-HIGH' else ''}>MEDIUM-HIGH</option>
                        <option value="MEDIUM" {'selected' if confidence_filter == 'MEDIUM' else ''}>MEDIUM</option>
                        <option value="LOW" {'selected' if confidence_filter == 'LOW' else ''}>LOW</option>
                    </select>
                </div>
                ...
                <div class="filter-group">
                    ...
                    <select id="status-filter" class="filter-select" onchange="applyFilters()">
                        ...
                        <option value="EXECUTED" {'selected' if status_filter == 'EXECUTED' else ''}>Ejecutadas</option>
                        <option value="PROPOSED" {'selected' if status_filter == 'PROPOSED' else ''}>Propuestas</option>
                        <option value="REJECTED" {'selected' if status_filter == 'REJECTED' else ''}>Rechazadas</option>
                    </select>
                </div>
            </div>
